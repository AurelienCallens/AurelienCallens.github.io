---
title: Can Data Science make me a better angler?
author: ''
date: '2020-09-19'
slug: DS_fishing_part_1
categories:
  - R
tags:
  - shiny
  ...
  - Web scraping
type: ''
bigimg: [{src: "/img/seabass_bay.png", desc: "Seabass"}]
subtitle: ''
---
...
# In this post, I will show you how to build from scratch a shiny application that collects data about meteorological conditions and fish caught during a fishing session. First, I will explain the reasons that lead me to develop this application. Then I will show you how I collect data from different sources (API and web scrapping) to improve my fishing results. The data collected last year is cleaned and explored in a different post (accessible *here*).
```



## Fishing in estuaries: too much variability! 

My favourite hobby, in addition to R of course, is fishing. Most of the time, I am fishing european sea bass (*Dicentrarchus labrax*) in estuaries. The sea bass is a predatory fish that has a broad range of preys: crabs, sand eels, prawns, shrimps and other fish. To catch these predators, I don't use live baits, I prefer to use artficial lures that imitate a specific prey. 
...
In pratice, it is an other story ! Indeed, the feeding acitivity, the position of the european sea bass in the estuary and their preys will vary depending on different parameters : 

  * the local substrat, which will depend where I fish
...
| Lures | Type, length, color of lure used | User input |
  ...
  + It must be adapted to small screens because I will alway use the application on my phone. 
...
My gps location is collected by using a bit of Javascript in the header of the shiny application. This code has been developped by AugusT and is available on his [github repository](https://github.com/AugustT/shiny_geolocation). 
...
The main problem I encountered while developping this application was data storage without paying. Shinyapps
host freely your shiny application but you can't store and modify csv files on their server. 
The solution I found was to store the data in my dropbox account, you can find [here](https://shiny.rstudio.com/articles/persistent-data-storage.html) more details on the subject and alternatives solutions. I used the package *rdrop2* to access and modify the data with the shiny application. 
...
In the coming months, I will improve this application on two points: the weather API and the tide function. I must find a new free API to replace the actual one. Indeed, the weather API I am using has been bought by Apple and the free requests will be stopped in the following year. For the tide function, the french website is now furnishing a data stream. Using the datastream instead of webscraping the tide levels should reduce the computation time of the function. 
